    ...
    <div class="container-fluid">
        ...
        <div class="col-md-6 col-lg-6 col-xl-4 box-col-6">
          <div class="card custom-card p-0">
            <div class="card-header">
              <img class="img-fluid" src="{{ asset('simpe_image.jpg') }}" alt="">
            </div>
                @if (isset($user->profile))
                    <div class="card-profile">
                      <img class="rounded-circle" src=" {{ asset('utilisateurs/images/image_profile/'.$user->profile->photo) }}" alt="">
                    </div>
                @else
                    <div class="card-profile">
                      <img class="rounded-circle" src="{{ asset('simpe_image.jpg') }}" alt="">
                    </div>
                @endif 
      ...
      <div class="col-sm-6 col-md-6  col-xl-6">
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
                <form class="needs-validation" action="{{ route('profile.gestionnaire.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  @if (session('status'))
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="needs-validation" action="{{ route('profile.gestionnaire.update', $user->id) }}" method="POST" enctype="multipart/form-data">
              @csrf
              ...
              <div class="row g-3">
                <div class="col-md-4">
                  ...
                  <input class="form-control" id="validationCustom01" type="text" name="first_name" value="{{ $user->profile->first_name ?? "Nom" }}">
                  @error('first_name')
                    <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
                <div class="col-md-4">
                  ...
                  <input class="form-control" id="validationCustom01" type="text" name="last_name" value="{{ $user->profile->last_name ?? "Prénom" }}">
                  @error('last_name')
                    <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
              ...
              <div class="row mt-3">
                <div class="col-md-4">
                  ...
                  <input class="form-control" id="validationCustom01" type="text" name="adress" value="{{ $user->profile->adress ?? "Adresse" }}">
                  @error('adress')
                    <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
                <div class="col-md-4">
                  ...
                  <input class="form-control" id="validationCustom01" type="number" name="phone" value="{{ $user->profile->phone ?? "525631852" }}">
                  @error('phone')
                    <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
