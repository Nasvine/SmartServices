...
require_once 'config.php';   
  ...
  case 'signup':  
    ...
    if($stmt->num_rows > 0){  
      ...
      $stmt = $conn->prepare("INSERT INTO users (email, role_id, password,remember_token) VALUES (?, ?, ?, ?)");
      $stmt->bind_param("ssss", $email, $role_id, $hashedPassword, $token); 
        ...
        if($stmt->execute()){  
            ...
            $confirmationLink = "http://anycourse.bj/config_anold/confirmation.php?token=" . $token;

            function mail_validation_inscription($email){
                // conversion de l'email en hash md5. Ce hash est a joindre à l'adresse de validation de l'abonnement
                $hash = md5($email);
                
                //message_confirmation est a envoyer a l'abonné 
                $headers_abo ='From: "Any course"<anycourse@anycourse.bj>'."\n";
                $headers_abo .='Reply-To: anycourse@anycourse.bj'."\n";
                $headers_abo .='Content-Type: text/html; charset="utf-8"'."\n";
                $headers_abo .='Content-Transfer-Encoding: 8bit';
                $message_confirmation =' <html>
                    ...
                    mail($email, 'Any course - Confirmez votre email', $message_confirmation, $headers_abo);
                    return true;
            
                }

                mail_validation_inscription($email);
...
case 'get_user':  
  ...
  case 'update_include_player_ids':  
    if(isTheseParametersAvailable(array('email', 'password','include_player_ids'))){  
      $email = $_POST['email'];
      $password = $_POST['password'];
      ...
      if ($stmt->num_rows > 0) {
          ...
          if (password_verify($password, $storedPassword)) {
              $stmt = $conn->prepare("UPDATE users SET include_player_ids = ? WHERE email = ?");
              $stmt->bind_param("ss", $include_player_ids,$email);
              $stmt->execute();
